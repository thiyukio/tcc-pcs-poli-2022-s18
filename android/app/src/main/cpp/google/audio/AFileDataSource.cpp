...
AFileDataSource* AFileDataSource::newFromFile(
        int fd,
        int length,
...
#else
    const long maximumDataSizeInBytes = kMaxCompressionRatio * (long) length * sizeof(int16_t);
    auto decodedData = new uint8_t[maximumDataSizeInBytes];

    int64_t bytesDecoded = NDKExtractor::decode(fd, length, decodedData, targetProperties);
    ...
    return new AFileDataSource(std::move(outputBuffer),
            numSamples,
            targetProperties);
}